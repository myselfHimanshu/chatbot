...
lines = open(data_path+"atis.train.w-intent.iob", encoding="utf-8", errors="ignore").read().split("\n")


#split the questions with answers
lines_split = []
for line in lines:
    _line = line.split("\t")[:2]
    lines_split.append(_line) 
...
#Doing the cleanup of the sentences:
def clean_text(text):
...
#Creating the dictionaries that map the questions words and the answers words to a unique integer
threshold = 0
questionswords2int = {}
word_number = 0
...
answersints2word = {i:w for w,i in answerswords2int.items()}

#Adding the EOS to the end of every answer
...
def decode_training_set(encoder_state, decoder_cell, decoder_embedded_input,sequence_length, 
    ...
    #keys:keys to compare with targets, 
    #values: to construct the context, 
    #score: similarity with keys and targets, 
    #construct: fun use to build attention state 
    attention_keys, attention_values, \
    attention_score_function, attention_construct_function = tf.contrib.seq2seq.prepare_attention \
...
def decoder_rnn(decoder_embedded_inputs, decoder_embeddings_matrix, encoder_state, num_words, sequence_length,
...
def seq2seq_model(inputs, targets, keep_prob, batch_size, sequence_length, answers_num_words, 
    ...
    training_predictions, test_predictions = decoder_rnn(decoder_embedded_inputs, decoder_embeddings_matrix,
                                                         encoder_state, questions_num_words, sequence_length,
                                                         rnn_size, num_layers, questionswords2int, keep_prob, 
                                                         batch_size)
    return training_predictions, test_predictions
...
training_predictions, test_predictions = seq2seq_model(tf.reverse(inputs,[-1]), targets, keep_prob,
                                                       batch_size, sequence_length, len(answerswords2int),
...
def apply_padding(batch_of_sequences, word2int):
    max_sequence_length = max([len(i) for i in batch_of_sequences])
    return [i+[word2int['<PAD>']]*(max_sequence_length-len(i)) for i in batch_of_sequences]
...
#Splitting the data into training and test/validation data
    ...
    for batch_index, (padded_questions_in_batch, padded_answers_in_batch) in enumerate(split_into_batches(training_questions, training_answers, batch_size)):
        ...
        if batch_index%batch_index_check_training_loss==0:
            print("Epoch: {:>3}/{}, Batch: {:>4}/{}, Training Loss Error: {:>6.3f}, Training Time on 100 Batches: {:d} seconds".format(epoch,epochs,
                  batch_index, len(training_questions)//batch_size, total_training_loss_error/batch_index_check_training_loss,
        ...
        if batch_index%batch_index_check_validation_loss==0 and batch_index>0:
            ...
            if average_validation_loss_error<=min(list_validation_loss_error):
                ...
                print("I do not speak better, need to practice more")
